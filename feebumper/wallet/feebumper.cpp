                         ...
                         CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx)
{
    ...
    if (total_fee > 0) {
        CAmount minTotalFee = nOldFeeRate.GetFee(maxNewTxSize) + ::incrementalRelayFee.GetFee(maxNewTxSize);
			  // 설정으로 입력받은 total_fee가 minTotalFee보다 작을 경우
    ...
    // moment earlier. In this case, we report an error to the user, who may use total_fee to make an adjustment.
	  // mempool에 포함되기 위한 최소한의 feerate 조건을 만족시키는지 확인.
    ...
    CTxOut* poutput = &(mtx.vout[nOutput]);
	  // 기존에 설정된 잔돈이 추가로 내야할 fee보다 적을 경우.
    ...
    poutput->nValue -= nDelta;
	  // 새로 계산한 잔돈이 dustThreshold 보다도 작을 경우 그냥 잔돈 안받음.
    ...
    // Mark new tx not replaceable, if requested.
	  // 새로운 tx가 replacable하지 않도록 설정할 경우 nSequence값 강제.
